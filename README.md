# Manual PIA Wireguard VPN Connections. Debian 12

### This is a fork of the original Linux scripts at https://github.com/glorious1/manual-connections  

Fork Notes:

1. Pia Wireguard namual conecction on Debian 12 (Tested on Debian 12.9, maybe works in other linux)
2. Full configuration on config/pia_config.json
3. Install on /opt/pia
4. Only Wireguard protocol are supported
5. New option in config/pia_config.json: Region ="auto" or "DE Berlin" / "ES Valencia" ...
6. New option in config/pia_config.json in order to ignore geolocalizated Regions
7. I recomend start `run.sh` with an @reboot root cron job:
	`@reboot /opt/pia/run.sh > /var/log/pia_startup.log 2>&1`
